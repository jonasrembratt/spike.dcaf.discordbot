...
namespace dcaf.discordbot
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            s_client.Log += onLog;
            // s_client.MessageReceived += onMessageReceived;

            
            var isCliEnabled = isCli(args, out var command, out var cliParams); 
            s_services = await s_client.ConfigureServicesAsync(cts, isCliEnabled);
            s_log = s_services.GetService<ILog>();
            var dcafConfig = s_services.GetRequiredService<DcafConfiguration>();
            var personnel = s_services.GetRequiredService<IPersonnel>();
